...
lunr.Index.loadPacked = function(serialisedData) {
  ...
  for (var i=0; i<packedTokens.length; i++) {
    ...
    for (var j=0; j<docIds.length; j++) {
      ...
      idx.tokenStore.add(token, {ref: docName, tf: tfs[j]});
 ...
 *
 * In the packed array, negative integers are immediate values
 * to be parsed as the positive version of the integer.
 * Positive numbers in the packed arrays are offsets from the previous number to the current number.
 *
 ...
 * @memberOf Index
 */
lunr.Index.prototype.toPackedJSON = function() {
